...
export const createUser = async ( req: Request, res: Response ): Promise<Response> => {
    ...
    try {
        ...
        const { username, fullname, faculty, cycle, dni, email, password }: User = req.body;
        ...
        await conn.query(`CALL SP_REGISTER_USER(?,?,?,?,?,?,?,?,?,?);`, [ uuidv4(), fullname, username,faculty,cycle, dni, email, pass, uuidv4(), randomNumber ]);
...
export const updateProfile = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        const { user, description, fullname, phone, cycle }: IUpdateProfile = req.body;

        const conn = await connect();

        await conn.query('UPDATE users SET username = ?, description = ? WHERE person_uid = ?', [ user, description, req.idPerson ]);

        await conn.query('UPDATE person SET fullname = ?, phone = ?, cycle = ? WHERE uid = ?', [ fullname, phone, cycle, req.idPerson ]);
