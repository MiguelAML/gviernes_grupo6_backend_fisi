...
DELIMITER //
CREATE PROCEDURE SP_REGISTER_USER(IN uidPerson varchar(100), IN full_name varchar(100),IN usernamed varchar(50), IN emailaddres varchar(100),IN pass varchar(100), IN uidUser varchar(100),IN temp varchar(50))
BEGIN
	INSERT INTO person(uid, fullname, image) VALUE (uidPerson, full_name, 'avatar-default.png');

	INSERT INTO users(uid, username, email, passwordd, person_uid, token_temp) VALUE (uidUser, usernamed, emailaddres, pass, uidPerson, temp);
END//
...
BEGIN
	...
	GROUP BY img.post_uid, co.post_uid, pos.created_at 
